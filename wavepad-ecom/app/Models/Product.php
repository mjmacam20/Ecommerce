...
class Product extends Model
{
    ...
    public static function getDiscountPrice($product_id){
        ...
        if($proDetails['product_discount']>0){
            //If product discount is added from the admin panel calculate
            $discounted_price = $proDetails['product_price'] - ($proDetails['product_price']*$proDetails['product_discount']/100);
        }else if($catDetails['category_discount']>0){
            //If product discount is not added but category discount added from the admin panel
            $discounted_price = $proDetails['product_price'] - ($proDetails['product_price']*$catDetails['product_discount']/100);
        }else{
            $discounted_price = 0;
        }
